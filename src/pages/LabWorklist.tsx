...
import { listLabWorklist, listLabHistory } from '@/api/lab';
import { resolveError } from '@/utils/errorMessages';
import { labSpecimenStatusLabel, orderStatusLabel } from '@/utils/labels';
...
export default function LabWorklist() {
  ...
  const [historyItems, setHistoryItems] = useState<LabWorklistItem[]>([]);
  ...
  const fetchHistory = useCallback(async () => {
    ...
    setHistoryError('');
    try {
      const result = await listLabHistory({
        page: historyPage,
        ...
        <div>
          ...
          <p className="mt-1 mb-0 text-[15px] text-[#6c757d]">
            {view === 'active'
              ...
              : 'Các chỉ định xét nghiệm đã hoàn tất hoặc đã hủy'}
          </p>
        </div>
        {view === 'active' && (
          <MultiSelect
        ...
        <>
          ...
          {historyLoading ? (
            ...
            <div className="p-15 text-center text-[#6c757d]">
              ...
              <h3 className="mb-2 text-[#274760]">Không có chỉ định xét nghiệm nào trong khoảng thời gian này</h3>
            </div>
          ) : (
            <div className="flex flex-col gap-3.5">
