...
import {
  getSupplyStockAudit, updateSupplyStockAuditItem, finalizeSupplyStockAudit,
} from '@/api/medicalSupply';
...
import type { SupplyStockAudit, SupplyStockAuditItem } from '@/components/medical-supplies/types';

export default function SupplyStockAuditDetail() {
  const { id = '' } = useParams();
  const [audit, setAudit] = useState<SupplyStockAudit | null>(null);
  ...
  const fetchAudit = useCallback(async () => {
    ...
    setError('');
    try {
      const result = await getSupplyStockAudit(id);
  ...
  const handleFinalize = async () => {
    ...
    setError('');
    try {
      await finalizeSupplyStockAudit(id);
      await fetchAudit();
    } catch (err) {
      setError(resolveError(err));
    } finally {
  ...
  return (
    <>
      <Link to="/medical-supplies/stock-audits" className="mb-4 inline-flex items-center gap-1.5 text-sm font-semibold text-[#307bc4] no-underline">
      ...
      <Card className="rounded-2xl border-[#e8edf2] p-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            {isDraft && (
              <Button
                ...
                title={incompleteCount > 0 ? 'Cần nhập đủ số đếm thực tế cho tất cả vật tư trước khi hoàn tất' : undefined}
                size="cta"
              >
        ...
        {isDraft && incompleteCount > 0 && (
          <p className="mt-3.5 mb-0 text-[13px] text-[#8a6100]">
            Còn {incompleteCount} vật tư chưa nhập số đếm thực tế.
          </p>
        )}
        {error && (
      ...
      <Card className="mt-5 gap-0 overflow-hidden rounded-2xl border-[#e8edf2] py-0">
        {items.length === 0 ? (
          <div className="p-15 text-center text-[#6c757d]">Không có vật tư nào trong đợt kiểm kê này.</div>
        ) : (
          <Table>
            <TableHeader>
...
function AuditItemRow({ auditId, item, editable, onSaved }: AuditItemRowProps) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await updateSupplyStockAuditItem(auditId, item.ID, { counted_quantity: Number(counted) });
      await onSaved();
    } catch (err) {
  ...
  return (
    <TableRow className="border-t border-[#f0f4f8]">
      <TableCell className="px-4 py-3 text-sm font-semibold text-[#274760]">{item.Supply?.Name ?? `#${item.SupplyID}`}</TableCell>
