...
import { z } from 'zod';
import { createSupplyStockAudit, listSupplyStockAudits } from '@/api/medicalSupply';
...
import type { SupplyStockAudit } from '@/components/medical-supplies/types';
...
export default function SupplyStockAudits() {
  const navigate = useNavigate();
  const [audits, setAudits] = useState<SupplyStockAudit[]>([]);
  ...
  const fetchAudits = useCallback(async () => {
    ...
    setError('');
    try {
      const result = await listSupplyStockAudits({
        q: search || undefined,
  ...
  const handleCreate = handleSubmit(async values => {
    ...
    try {
      ...
      const result = await createSupplyStockAudit({ audit_date: auditDate, notes: values.notes.trim() });
      setModalOpen(false);
      navigate(`/medical-supplies/stock-audits/${result.data.ID}`);
    } catch (err) {
  ...
  return (
    <>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="m-0 text-[26px] font-bold text-[#274760]">Kiểm kê kho vật tư</h1>
          <p className="mt-1 mb-0 text-[15px] text-[#6c757d]">Các đợt kiểm kê định kỳ và xử lý chênh lệch tồn kho vật tư</p>
        </div>
      ...
      <Card className="gap-0 overflow-hidden rounded-2xl border-[#e8edf2] py-0">
        {loading ? (
          ...
          <Table>
            ...
            <TableBody>
              {audits.map(a => (
                <TableRow key={a.ID} onClick={() => navigate(`/medical-supplies/stock-audits/${a.ID}`)} className="cursor-pointer border-t border-[#f0f4f8]">
        ...
        <DialogContent className="sm:max-w-[460px] rounded-[20px] p-8">
          ...
          <form onSubmit={handleCreate} noValidate>
            <p className="mt-4 mb-0 text-sm text-[#6c757d]">
              Hệ thống sẽ chụp lại tồn kho hiện tại của toàn bộ danh mục vật tư để đối chiếu khi kiểm đếm.
            </p>
