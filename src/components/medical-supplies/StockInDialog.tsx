...
import { supplyStockInSchema, type SupplyStockInFormValues } from '@/schemas/medicalSupply';
import { stockInSupply } from '@/api/medicalSupply';
import { resolveError } from '@/utils/errorMessages';
import type { MedicalSupply } from './types';

const EMPTY_FORM: SupplyStockInFormValues = {
  lot_number: '',
  expiry_date: '',
  quantity: 0,
  supplier: '',
...
export default function StockInDialog({ open, supply, onClose, onSaved }: StockInDialogProps) {
  const [formError, setFormError] = useState('');
  const [saving, setSaving] = useState(false);
  const {
    register, control, handleSubmit, reset, formState: { errors },
  } = useForm<SupplyStockInFormValues>({
    resolver: zodResolver(supplyStockInSchema),
