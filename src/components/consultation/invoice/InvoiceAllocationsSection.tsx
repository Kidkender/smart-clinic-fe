...
import { splitInvoice, submitClaim, recordClaimResponse } from '@/api/billing';
...
export default function InvoiceAllocationsSection({ invoice, payers, encounterId, refreshInvoice, onBusyChange }: InvoiceAllocationsSectionProps) {
  ...
  // insurer, and its amount has to come out of the total before figuring out
  // what's left for the patient/other payers to split.
  const bhytAllocation = allocations.find(a => a.Payer?.Type === 'government');
  ...
  const handleSubmitClaim = async (e: FormEvent) => {
    ...
    setError('');
    if (!(await confirm(`Xác nhận gửi yêu cầu bồi thường cho "${allocationPayerLabel(claimAllocation)}"?`, { confirmLabel: 'Xác nhận', danger: false }))) {
      return;
    }
  ...
  const handleSubmitClaimResponse = async (e: FormEvent) => {
    ...
    if (claimResponseStatus === 'approved') {
      ...
      if (parsedApprovedAmount > claimResponseAllocation.Claim.SubmittedAmount) {
        setError('Số tiền duyệt không được vượt quá số tiền đã gửi yêu cầu bồi thường.');
        return;
      }
    }
    setError('');
    if (!(await confirm('Xác nhận ghi nhận kết quả duyệt yêu cầu bồi thường?', { confirmLabel: 'Xác nhận', danger: false }))) {
      return;
    }
    ...
    <div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Bồi thường</TableHead>
      ...
      <Dialog open={!!claimResponseAllocation} onOpenChange={o => { if (!o) closeClaimResponseDialog(); }}>
        <DialogContent className="sm:max-w-[480px] rounded-[20px] p-8">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-[#274760]">Ghi nhận kết quả duyệt yêu cầu bồi thường</DialogTitle>
          </DialogHeader>
